...
public class ControladorREST {
    ...
    public List<Reserva> listarMisReservas(
        ...
        // 3. Delegar al servicio para obtener las reservas del usuario
        return reservaService.obtenerMisReservas(userId, fromDate, toDate);
    }
    ...
    private boolean isPistaLibreEnFecha(Long courtId, LocalDate fecha) {
        List<Reserva> reservasEnPista = reservaService.obtenerTodasReservas().stream()
                .filter(r -> r.courtId.equals(courtId))
                .toList();
        return reservasEnPista.stream()

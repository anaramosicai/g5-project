...
*/
    ...
    @Test
    void disponibilidadSinReservas_devuelveFranjasLibres() {
        HttpHeaders headers = new HttpHeaders();
        headers.setBasicAuth("admin", "clave");

        // Crear una pista
        Pista pista = new Pista(0L, "Pista Disp E2E", "Indoor", 3500, true, "2026-01-01");
        ...
        ResponseEntity<String> resp = restTemplate.exchange(
                "http://localhost:" + port + "/courts/" + courtId + "/availability?date=" + today,
                HttpMethod.GET,
                new HttpEntity<>(headers),
                String.class
        );
    ...
    @Test
    void disponibilidadGeneral_devuelveListaDeDisponibilidades() {
        HttpHeaders headers = new HttpHeaders();
        headers.setBasicAuth("admin", "clave");
        LocalDate today = LocalDate.now();

        // Consultar disponibilidad general sin courtId
        ResponseEntity<String> resp = restTemplate.exchange(
                "http://localhost:" + port + "/availability?date=" + today,
                HttpMethod.GET,
                new HttpEntity<>(headers),
                String.class
        );
        ...
        assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(resp.getBody()).isNotNull();
        assertThat(resp.getBody()).contains("["); // Es una lista JSON
    }

    @Test
    void disponibilidadConCourtInvalido_devuelve404() {
        HttpHeaders headers = new HttpHeaders();
        headers.setBasicAuth("admin", "clave");
        LocalDate today = LocalDate.now();
        Long courtIdInvalido = 99999L;

        // Consultar disponibilidad de pista que no existe
        ResponseEntity<String> resp = restTemplate.exchange(
                "http://localhost:" + port + "/courts/" + courtIdInvalido + "/availability?date=" + today,
                HttpMethod.GET,
                new HttpEntity<>(headers),
                String.class
        );
    ...
    @Test
    void disponibilidadSinFecha_devuelve400() {
        HttpHeaders headers = new HttpHeaders();
        headers.setBasicAuth("admin", "clave");

        // Crear una pista
        Pista pista = new Pista(0L, "Pista BadReq", "Indoor", 3500, true, "2026-01-01");
        ResponseEntity<Pista> pistaResp = restTemplate.exchange(
                "http://localhost:" + port + "/pistaPadel/courts",
                HttpMethod.POST,
                new HttpEntity<>(pista, headers),
        ...
        // Consultar SIN parámetro de fecha
        ResponseEntity<String> resp = restTemplate.exchange(
                "http://localhost:" + port + "/courts/" + courtId + "/availability",
                HttpMethod.GET,
                new HttpEntity<>(headers),
                String.class
        );
